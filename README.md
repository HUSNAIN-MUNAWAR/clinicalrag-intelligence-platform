...
The safety classifier is deterministic and intentionally conservative. It is not a clinical validation layer. It blocks known unsafe categories but should be expanded and audited before any real deployment.

## Known limitations

- Default embeddings are local deterministic hash embeddings for zero-cost reproducibility, not state-of-the-art biomedical embeddings.
- Default vector store is local JSON for easy portfolio demonstration; Qdrant/pgvector integration is an extension point.
- LLM generation defaults to retrieval-only extractive answers unless external provider env vars are configured.
- Demo corpus is small and synthetic; external corpora must be downloaded and licensed responsibly.
- Not clinically validated and not intended for patient care.

## Roadmap

- SentenceTransformers biomedical embedding provider
- Qdrant vector backend implementation
- PubMed E-utilities adapter with proper email/tool/api-key config and rate limiting
- Production-scale PMC AWS/eSearch downloader with topic filters and resumable inventory processing
- NER-based medical entity graph
- Async worker queue with Celery/RQ
- Human evaluation UI for answer grading
- OpenTelemetry tracing and cost telemetry
- Cloud deployment guide

## Resume bullets

- Built a production-grade Advanced RAG platform for biomedical knowledge using FastAPI, React, vector search, BM25, hybrid retrieval, reranking, and citation-grounded generation.
- Implemented multiple RAG strategies including parent-child retrieval, multi-query retrieval, RAG-Fusion, section-aware retrieval, and router-based pipeline selection.
- Developed a medical document ingestion pipeline for open biomedical corpora with license validation, metadata extraction, medical-aware chunking, embedding generation, and index management.
- Built an evaluation lab measuring Recall@K, MRR, nDCG, citation coverage, groundedness, hallucination risk, and latency across RAG pipelines.
- Designed safety guardrails to detect and refuse diagnosis, prescription, dosage, emergency, and personalized medical advice requests.

## Interview talking points

- Why medical RAG needs citation grounding and refusal logic.
- Why hybrid retrieval beats vector-only for exact clinical terminology.
- How parent-child retrieval improves context without losing precise matching.
- How section-aware scoring helps prioritize abstract, warnings, diagnosis, contraindications, and treatment sections.
- How you would replace local demo embeddings/indexes with biomedical embeddings and Qdrant in production.
- How to measure retrieval quality separately from answer quality.
- How to avoid unsafe clinical claims and illegal data use.
